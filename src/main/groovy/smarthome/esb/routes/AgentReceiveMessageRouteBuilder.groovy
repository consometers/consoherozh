...
class AgentReceiveMessageRouteBuilder extends RouteBuilder {
	...
	void configure() throws Exception {
		...
		.choice()
		.when(simple('${exchangeProperty.header} == "deviceValue"'))
			.setProperty("datas").groovy('body.arg0.data')
			.setProperty("agentId").groovy('body.result.id')
			.setProperty("agent").method("agentService", "findById(echangeProperty.agentId)")
			.to("bean:deviceService?method=changeValueFromAgent(exchangeProperty.agent, exchangeProperty.datas)")
		.when(simple('${exchangeProperty.header} == "shell"'))
		...
		.when(simple('${exchangeProperty.header} == "teleinfo-trace" || ${exchangeProperty.header} == "teleinfo-trace-stop"'))
			.marshal().json(JsonLibrary.Gson)
